...
SCENARIOS = {
    "elevator": {
        "normal": {"vibration": 1, "temperature": 25, "speed_var": 1, "amp": 0.01},
        "desgaste_poleas": {"vibration": 6, "temperature": 45, "speed_var": 4, "amp": 0.15,
                            "label": "Desgaste de Poleas", "icon": "tools"},
        "sobrecalentamiento": {"vibration": 3, "temperature": 85, "speed_var": 3, "amp": 0.1,
                               "label": "Sobrecalentamiento", "icon": "thermometer"},
        "falla_motor": {"vibration": 9, "temperature": 70, "speed_var": 8, "amp": 0.2,
                        "label": "Falla de Motor", "icon": "alert-triangle"},
    },
    "electrical": {
        "normal": {"voltage": 220, "current": 20, "temp_rise": 20, "amp": 0.01},
    ...
    "water": {
        "normal": {"flow": 50, "pressure": 50, "tank_level": 70, "amp": 0.01},
        "fuga_tuberia": {"flow": 10, "pressure": 20, "tank_level": 25, "amp": 0.15,
                         "label": "Fuga en Tubería", "icon": "droplet"},
        "bomba_danada": {"flow": 5, "pressure": 15, "tank_level": 40, "amp": 0.12,
                         "label": "Bomba Dañada", "icon": "alert-circle"},
        "tanque_vacio": {"flow": 30, "pressure": 40, "tank_level": 5, "amp": 0.1,
                         "label": "Tanque Vacío", "icon": "alert-triangle"},
    },
    "hvac": {
        "normal": {"temp_in": 24, "temp_out": 12, "pressure": 120, "rpm": 1800, "amp": 0.01},
    ...
    "fire": {
        "normal": {"smoke_level": 1, "temp_rise": 22, "water_pressure": 100, "amp": 0.01},
    ...
    "generator": {
        "normal": {"fuel_level": 90, "rpm": 1800, "coolant_temp": 70, "voltage_out": 215, "amp": 0.01},
        "bajo_combustible": {"fuel_level": 12, "rpm": 1800, "coolant_temp": 70, "voltage_out": 218, "amp": 0.08,
                             "label": "Bajo Combustible", "icon": "fuel"},
        "sobrecalentamiento": {"fuel_level": 60, "rpm": 1900, "coolant_temp": 105, "voltage_out": 225, "amp": 0.12,
                               "label": "Sobrecalentamiento", "icon": "thermometer"},
        "falla_arranque": {"fuel_level": 80, "rpm": 200, "coolant_temp": 25, "voltage_out": 120, "amp": 0.2,
                           "label": "Falla de Arranque", "icon": "alert-circle"},
    },
    "lighting": {
        "normal": {"ambient_light": 50, "current_draw": 20, "voltage": 220, "amp": 0.01},
    ...
    "access": {
        "normal": {"door_sensors": 0, "motion_detect": 0, "battery_level": 95, "amp": 0.01},
...
RECOMMENDATIONS = {
    ...
    "water": {
        "critical": [
            "ACTIVAR BOMBA DE EMERGENCIA",
            "Notificar a Junta de Condominio urgente",
            "Cortar suministro de agua y revisar tubería principal",
            "Riesgo de desabastecimiento total",
        ],
        "high": [
            "Inspeccionar bombas y motor eléctrico",
            "Revisar válvulas y filtros del sistema",
            "Monitorear nivel de tanque cada 30 min",
            "Programar reparación en 24h",
        ],
        "medium": [
            "Revisar presión de la bomba principal",
            "Verificar estado de flotadores del tanque",
            "Monitorear consumo de agua",
            "Mantenimiento preventivo en 30 días",
        ],
        "low": [
            "Revisar niveles de cloro y pH",
            "Limpieza programada de tanques",
            "Sistema operando normalmente",
        ],
        "none": ["Sistema de agua en óptimas condiciones"],
    },
    "hvac": {
        "critical": [
    ...
    "generator": {
        ...
        "low": [
            ...
            "Mantenimiento rutinario programado",
        ],
...
class IIoTSimulator:
    ...
    def set_scenario(self, asset, scenario_name, auto_recover=True):
        ...
        """
        if asset in self.scenarios and scenario_name in SCENARIOS.get(asset, {}):
    ...
    def get_scenarios_list(self, asset):
        ...
        return [
            ...
            for k, v in SCENARIOS.get(asset, {}).items()
        ]


    def _read(self, asset):
        ...
        scenario_name = self.scenarios.get(asset, "normal")
    ...
    def read_all(self):
        """Read all sensors for all assets, then apply actuator effects.
    ...
    def apply_actuators(self, values, asset_name):
        ...
        if asset_name == "elevator":
            ...
            rv = cmds.get("relief_valve", 0)
            if rv > 0 and "pressure" in values:
                values["pressure"] *= (1 - rv / 100.0)
            if cmds.get("booster_pump", 0) > 0.5:
                if "pressure" in values:
                    values["pressure"] += 20
                if "flow" in values:
                    values["flow"] += 20
            if cmds.get("inlet_valve", 0) > 0.5:
                if "tank_level" in values:
                    values["tank_level"] = min(100, values["tank_level"] + 5)
            ...
            if cmds.get("fuel_valve", 1) < 0.5:
                if "rpm" in values:
                    values["rpm"] = max(0, values["rpm"] * 0.1)
                if "voltage_out" in values:
                    values["voltage_out"] = max(0, values["voltage_out"] * 0.1)
            if cmds.get("starter", 0) > 0.5:
                if "rpm" in values:
                    values["rpm"] = max(values["rpm"], 1700)
                if "voltage_out" in values:
                    values["voltage_out"] = min(230, values["voltage_out"] + 20)
            if cmds.get("load_transfer", 0) < 0.5 and "voltage_out" in values:
                values["voltage_out"] = 0
